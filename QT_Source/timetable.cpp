...
void Timetable::initNetWork() {
    ...
    this->roomID = settings.value("room/roomID", "").toString();
    ui->lineEdit_2->setText(roomID);

    ui->lineEdit->setText(currentDate.toString("yyyy-MMM-dd (dddd)"));
    QString url = "https://f0vafga1w2.execute-api.ap-southeast-1.amazonaws.com/api/venue/" + roomID + "/date/";
